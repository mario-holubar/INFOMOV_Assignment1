...
bool CheckCL(cl_int result, const char *file, int line)
{
	...
	if (result == CL_MAP_FAILURE)
		FatalError("Error: CL_MAP_FAILURE\n%s, line %i", file, line, "OpenCL error");
	...
	if (result == CL_EXEC_STATUS_ERROR_FOR_EVENTS_IN_WAIT_LIST)
		FatalError("Error: CL_EXEC_STATUS_ERROR_FOR_EVENTS_IN_WAIT_LIST\n%s, line %i", file, line, "OpenCL error");
	if (result == CL_INVALID_VALUE)
		FatalError("Error: CL_INVALID_VALUE\n%s, line %i", file, line, "OpenCL error");
	...
	if (result == CL_INVALID_DEVICE)
		FatalError("Error: CL_INVALID_DEVICE\n%s, line %i", file, line, "OpenCL error");
	if (result == CL_INVALID_CONTEXT)
		FatalError("Error: CL_INVALID_CONTEXT\n%s, line %i", file, line, "OpenCL error");
	...
	if (result == CL_INVALID_IMAGE_FORMAT_DESCRIPTOR)
		FatalError("Error: CL_INVALID_IMAGE_FORMAT_DESCRIPTOR\n%s, line %i", file, line, "OpenCL error");
	if (result == CL_INVALID_IMAGE_SIZE)
		FatalError("Error: CL_INVALID_IMAGE_SIZE\n%s, line %i", file, line, "OpenCL error");
	if (result == CL_INVALID_SAMPLER)
		FatalError("Error: CL_INVALID_SAMPLER\n%s, line %i", file, line, "OpenCL error");
	if (result == CL_INVALID_BINARY)
		FatalError("Error: CL_INVALID_BINARY\n%s, line %i", file, line, "OpenCL error");
	if (result == CL_INVALID_BUILD_OPTIONS)
		FatalError("Error: CL_INVALID_BUILD_OPTIONS\n%s, line %i", file, line, "OpenCL error");
	if (result == CL_INVALID_PROGRAM)
		FatalError("Error: CL_INVALID_PROGRAM\n%s, line %i", file, line, "OpenCL error");
	...
	if (result == CL_INVALID_KERNEL)
		FatalError("Error: CL_INVALID_KERNEL\n%s, line %i", file, line, "OpenCL error");
	...
	if (result == CL_INVALID_EVENT)
		FatalError("Error: CL_INVALID_EVENT\n%s, line %i", file, line, "OpenCL error");
...
Buffer::Buffer(unsigned int N, void *ptr, unsigned int t)
{
	...
	else
	{
		...
		if (!Kernel::candoInterop)
			FatalError("didn't expect to get here.");
		int error = 0;
		if (t == TARGET)
			deviceBuffer = clCreateFromGLTexture(Kernel::GetContext(), CL_MEM_WRITE_ONLY, GL_TEXTURE_2D, 0, N, &error);
		else
			deviceBuffer = clCreateFromGLTexture(Kernel::GetContext(), CL_MEM_READ_ONLY, GL_TEXTURE_2D, 0, N, &error);
		CHECKCL(error);
		hostBuffer = 0;
	}
}
...
Kernel::Kernel(char *file, char *entryPoint)
{
	...
	if (csText.size() == 0)
		FatalError("File %s not found", file);
	// add vendor defines
	vendorLines = 0;
	if (isNVidia)
		csText = "#define ISNVIDIA\n" + csText, vendorLines++;
	if (isAMD)
		csText = "#define ISAMD\n" + csText, vendorLines++;
	if (isIntel)
		csText = "#define ISINTEL\n" + csText, vendorLines++;
	if (isOther)
		csText = "#define ISOTHER\n" + csText, vendorLines++;
	if (isAmpere)
		csText = "#define ISAMPERE\n" + csText, vendorLines++;
	if (isTuring)
		csText = "#define ISTURING\n" + csText, vendorLines++;
	if (isPascal)
		csText = "#define ISPASCAL\n" + csText, vendorLines++;
	...
	else
	{
		...
		if (!log)
			log = new char[256 * 1024]; // can be quite large
		log[0] = 0;
		...
		if (errorString)
		{
			...
			if (!lns)
				FatalError(log + errorPos);
			else
			{
				lns += 2;
				while (*lns >= '0' && *lns <= '9')
					lineNr = lineNr * 10 + (*lns++ - '0');
				lns++; // proceed to line number
				while (*lns >= '0' && *lns <= '9')
					linePos = linePos * 10 + (*lns++ - '0');
				...
				*eol = 0;
				...
				for (int i = Ninc - 1; i >= 0; i--)
				{
					...
					{
						for (int j = 0; j <= i; j++)
							lineNr -= includes[j].end - includes[j].start;
						break;
					}
	...
	if (kernel == 0)
		FatalError("clCreateKernel failed: entry point not found.");
	CHECKCL(error);
}
...
bool Kernel::InitCL()
{
		...
		if (extensionSize > 0)
		{
			...
			for (int j = 0; j < 2; j++)
			{
				...
				while (s != deviceList.npos)
				{
					...
					if (strcmp(mustHave[j].c_str(), subs.c_str()) == 0)
						hasFeature = true;
					do
					{
						o = s + 1, s = deviceList.find(' ', o);
					} while (s == o);
				}
				if (!hasFeature)
					hasAll = false;
			...
			if (hasAll)
			{
				...
				auto glDisplay = glXGetCurrentDisplay();
				cl_context_properties props[] = {
					CL_GL_CONTEXT_KHR, (cl_context_properties)glContext,
					CL_GLX_DISPLAY_KHR, (cl_context_properties)glDisplay, // Use GLX display on Linux
				...
				cl_context_properties props[] = {
					CL_GL_CONTEXT_KHR, (cl_context_properties)glContext,
					CL_WGL_HDC_KHR, (cl_context_properties)hdc, // Use HDC on Windows
					CL_CONTEXT_PLATFORM, (cl_context_properties)platform,
					0};
#else
				// macOS specific code
				CGLContextObj glContext = CGLGetCurrentContext();
				CGLPixelFormatObj glPixelFormat = CGLGetPixelFormat(glContext);
				cl_context_properties props[] = {
					CL_GL_CONTEXT_KHR, (cl_context_properties)glContext,
					CL_CGL_SHAREGROUP_KHR, (cl_context_properties)CGLGetShareGroup(glContext), // Use CGL share group on macOS
					CL_CONTEXT_PLATFORM, (cl_context_properties)platform,
					0};
#endif
	...
	if (deviceUsed == -1)
		FatalError("No capable OpenCL device found.");
	...
	if (strstr(d, "nvidia"))
	{
		...
		if (strstr(d, "rtx"))
		{
			...
			if (strstr(d, "3050") || strstr(d, "3060") || strstr(d, "3070") || strstr(d, "3080") || strstr(d, "3090"))
				isAmpere = true;
			if (strstr(d, "a2000") || strstr(d, "a3000") || strstr(d, "a4000") || strstr(d, "a5000") || strstr(d, "a6000"))
				isAmpere = true;
			// detect Turing GPUs
			if (strstr(d, "2060") || strstr(d, "2070") || strstr(d, "2080"))
				isTuring = true;
			// detect Titan RTX
			if (strstr(d, "titan rtx"))
				isTuring = true;
			// detect Turing Quadro
			if (strstr(d, "quadro"))
			{
				if (strstr(d, "3000") || strstr(d, "4000") || strstr(d, "5000") || strstr(d, "6000") || strstr(d, "8000"))
					isTuring = true;
			}
		}
		...
		{
			// detect Turing GPUs
			if (strstr(d, "1650") || strstr(d, "1660"))
				isTuring = true;
			// detect Pascal GPUs
			if (strstr(d, "1010") || strstr(d, "1030") || strstr(d, "1050") || strstr(d, "1060") || strstr(d, "1070") || strstr(d, "1080"))
				isPascal = true;
		}
		else if (strstr(d, "quadro"))
		{
			// detect Pascal GPUs
			if (strstr(d, "p2000") || strstr(d, "p1000") || strstr(d, "p600") || strstr(d, "p400") || strstr(d, "p5000") || strstr(d, "p100"))
				isPascal = true;
		}
		else
		{
	...
	if (isNVidia)
	{
		...
		if (isAmpere)
			...
			printf("PRE-PASCAL hardware (warning: slow).\n");
	}
	else if (isAMD)
	{
		printf("AMD.\n");
...
void Kernel::SetArgument(int idx, Buffer *buffer)
{
	...
	{
		if (acqBuffer)
			FatalError("Kernel can take only one texture target buffer argument.");
		acqBuffer = buffer;
	}
}
...
void Kernel::SetArgument(int idx, float3 value)
{
	CheckCLStarted();
	float4 tmp(value, 0);
	clSetKernelArg(kernel, idx, sizeof(float4), &tmp);
}
...
void Kernel::Run(const size_t count, const size_t localSize, cl_event *eventToWaitFor, cl_event *eventToSet)
{
	...
	{
		if (!Kernel::candoInterop)
			FatalError("OpenGL interop functionality required but not available.");
